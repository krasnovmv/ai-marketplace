...
⚠ Запись таблицы — находка, а не автоправка. Лишнее слово может нести противопоставление, и тогда его удаление меняет смысл: в `Экспонат выставки уехал в фонд, а экспонат постоянной экспозиции остался в зале` слово «выставки» различает два хранилища, а не повторяет значение. Механически противопоставление не проверить, поэтому колонка `correct` предлагается, а не применяется.
